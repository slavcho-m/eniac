...
```json
{
  ...
  "affected_files": ["relative/path/to/pipeline.yml", "relative/path/to/Dockerfile"],
  "out_of_scope": ["explicitly excluded thing"],
  "open_risks": ["anything uncertain the user or Assistants should know about"]
}
```
`requirements` must be concrete and testable — specific behavior an Assistant can implement directly against, not a restatement of the goal. `affected_files` are real paths you found by investigating, not guesses. `out_of_scope` and `open_risks` may be empty arrays, but must be present. Flag anything security- or credential-sensitive (secrets, access changes, prod deploy targets) as an `open_risk` even if you're otherwise confident.
...
```json
{
  "status": "ready",
  "tasks": [
    {"slug": "short-kebab-case-slug", "description": "what this task item covers, concrete enough for the Assistant to act on directly", "assistant": "CI-CD Implementer"}
  ]
}
```
`tasks` must be non-empty and ordered — earlier items should generally be done first. `assistant` must be exactly one of the three names above.
