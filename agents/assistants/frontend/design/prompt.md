...
- **`## Component Structure`** — new/changed components, their hierarchy, and where each lives.
- **`## State & Props`** — prop/type interfaces, state shape, data flow (which hook/service supplies what).
...
- **`## Conventions Followed`** — which existing pattern(s) you mirrored (name real files/components), and any deliberate deviation and why.
